...
logoElem.addEventListener('click',()=>{
    if(document.body.getAttribute("theme")==="slateblue-color"){
        document.body.setAttribute("theme", "red-color");
    }
    else{
        document.body.setAttribute("theme", "slateblue-color");
    }
    
});
// Toggle View
let mobileElem=document.getElementById('mobile-icon');
let navElems=document.getElementById("nav-elems");
mobileElem.addEventListener('click',()=>{
    if(mobileElem.classList.contains('fa-bars')){
        mobileElem.classList.replace("fa-bars","fa-close");
        navElems.style.left="-40px"
    }
    else{
        mobileElem.classList.replace("fa-close","fa-bars"); 
        navElems.style.left="-250px";
    }
})
navElems.addEventListener("click",()=>{
    navElems.style.left="-250px"
    mobileElem.classList.replace("fa-close","fa-bars"); 
})
// About Section
